...
source: "https://docs.aircode.run/docs/ccswitch/5-ccs_cli.html"
author:
  - "[[Packy Team]]"
published: 2026-05-22
created: 2026-05-31
description: "提示 CC Switch CLI 适合服务器、SSH、macOS 终端和自动化场景使用。如果你更习惯图形界面，可以继续使用前面的 CC-Switch 教程。 CC-Switch CLI Version Platform Built with Rust License Claude Code、Codex、Gemini、OpenCode 与 OpenCla..."
tags:
  - "clippings"
---
提示
...
![](assets/Pasted%20image%2020260531213553.png)
...
如果你只是第一次配置 aircode，推荐先用 TUI。配置完成后，日常切换、检查和排错可以直接用 CLI 命令完成。
...
![](assets/Pasted%20image%2020260531213627.png)

3. 在模板中选择 `* 自定义` 。


4. 在 `API Key` 中填入你从 aircode 复制的令牌，然后保存。


5. 回到供应商列表，确认当前选中的是刚刚添加的 。


6. 如果你配置的是 Claude Code，进入 `设置` ，找到 `跳过 Claude Code 初次安装确认` ，确认开启。
![](assets/Pasted%20image%2020260531213705.png)
