...
source: "https://docs.aircode.run/docs/faq/CC.html"
author:
  - "[[Packy Team]]"
published: 2026-05-22
created: 2026-05-31
description: "如何在Vscode CC插件中使用aircode Claude Code 中常用命令 claude code 无法连接到 Anthropic 服务 注意事项 如果你是首次配置根据链接跳转到这一步，直接按照下面的教程运行命令即可 使用npm安装完claude之后。 在命令行输入claude报了如下错误： 或是在你初次配置时出现以下问题： Claude ..."
tags:
  - "clippings"
---
### 如何在Vscode CC插件中使用aircode

Windows

1. 确保你已进行 [环境检查](../CLI配置教程/环境检查%28通用步骤%29.md) 并保证你的claude code cli是没问题的，如果有问题，请按照教程进行配置
2. 键盘按下“Win+R”键，输入以下内容后回车，打开Claude Code配置目录
```bash
%userprofile%\.claude
```
![](assets/Pasted%20image%2020260531212503.png)
3. 目录内容如图所示，如果目录中没有 `config.json` ，你需要手动创建后打开
- **config.json** ：配置Vscode的Claude Code插件的一些行为
![](assets/Pasted%20image%2020260531212457.png)

4. 在 `config.json` 中写入以下内容后保存
```json
{
  "primaryApiKey": "aircode"
}
```
...
~/.claude
```
![](assets/Pasted%20image%2020260531212518.png)

3. 目录内容如图所示，如果目录中没有 `config.json` ，你需要手动创建后打开
- **config.json** ：配置Vscode的Claude Code插件的一些行为
![](assets/Pasted%20image%2020260531212531.png)
...
```
![](assets/Pasted%20image%2020260531212545.png)

或是在你初次配置时出现以下问题：

![](assets/Pasted%20image%2020260531212551.png)
...
1. 在APP列表中找到终端程序，点击运行  
	![](assets/Pasted%20image%2020260531212636.png)
2. 在终端中运行以下命令后回车
```bash
